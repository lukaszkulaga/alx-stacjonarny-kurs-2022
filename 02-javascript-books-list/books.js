...
const booksList = document.querySelector('#book__list');
const addBookInput = document.querySelector('#addBookInput');
const addBookSelect = document.querySelector('#addBookSelect');
const button_search = document.querySelector('#button_search');

const renderBooks = () => {
    books.forEach(book =>{
        booksList.innerHTML += `<li>${book.title}${book.author}${book.website}</li>`
    })
}
...
const handleSubmit = (event) => {
    ...
    const inputValue = addBookInput.value;
    const selectValue  = addBookSelect.value;

    booksList.innerHTML = '';
    addBookInput.value = '';

    const bookFilterList= books.filter( (book) => {
            return book[selectValue].includes(inputValue);
    })
    .forEach(book =>{
        booksList.innerHTML += `<li>${book.title}${book.author}${book.website}</li>`
    })
...
button_search.addEventListener('click', handleSubmit);
